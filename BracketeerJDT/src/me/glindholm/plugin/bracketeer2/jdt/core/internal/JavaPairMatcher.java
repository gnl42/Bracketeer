...
public final class JavaPairMatcher extends DefaultCharacterPairMatcher implements ISourceVersionDependent {
    ...
    public JavaPairMatcher(final char[] pairs) {
    ...
    private boolean isLessThanOperator(final IDocument document, final int offset) throws BadLocationException {
        ...
        final JavaHeuristicScanner scanner = new JavaHeuristicScanner(document, IJavaPartitions.JAVA_PARTITIONING,
                TextUtilities.getContentType(document, IJavaPartitions.JAVA_PARTITIONING, offset, false));
     ...
     * @since 3.1
     */
    private boolean isTypeParameterBracket(final int offset, final IDocument document, final JavaHeuristicScanner scanner) {
        /*
         * type parameter come after braces (closing or opening), semicolons, or after a Type name
         * (heuristic: starts with capital character, or after a modifier keyword in a method declaration
         * (visibility, static, synchronized, final)
         */
        ...
        try {
            ...
            final String previous = prevToken == Symbols.TokenEOF ? null : document.get(prevTokenOffset, offset - prevTokenOffset).trim();

            if (prevToken == Symbols.TokenLBRACE || prevToken == Symbols.TokenRBRACE || prevToken == Symbols.TokenSEMICOLON
                    || prevToken == Symbols.TokenSYNCHRONIZED || prevToken == Symbols.TokenSTATIC
                    || prevToken == Symbols.TokenIDENT && isTypeParameterIntroducer(previous) || prevToken == Symbols.TokenEOF) {
                return true;
            }
     ...
     * @since 3.1
     */
    private boolean isTypeParameterIntroducer(final String identifier) {
