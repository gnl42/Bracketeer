...
public class PaintableBracket extends PaintableObject {
    ...
    public PaintableBracket(final Position position, final RGB foreground, final RGB background, final String highlightType) {
        super(position, foreground, highlightType.equals(PreferencesConstants.Highlights.HighlightTypeValSolid) ? background : null);
        _highlightType = highlightType;
        _outlineColor = background;
    }

    @Override
    protected void innerPaint(final GC gc, final StyledText st, final IDocument doc, final IRegion widgetRange, final Rectangle widgetRect) {
    ...
    public PaintableBracket clone(final Position newPos) {
